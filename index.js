...
app.post('/webhook', json({type: 'application/json'}), async (req, res) => {
  ...
  if (githubEvent === 'dependabot_alert') {
    ...
    if (action === 'created' || action === 'appeared_in_branch' || action === 'auto_reopened' || action === 'reopened') {
      ...
      const prompt = 'If the `jira_create_issue` tool is not available, immediately terminate the process. Write a Jira ticket for the Dependabot Issue details below. In the ticket, include any context a developer needs to remediate the Dependabot Issue.';
